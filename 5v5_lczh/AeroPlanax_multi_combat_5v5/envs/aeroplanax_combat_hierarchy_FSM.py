...
print(f'combat_hierarchy policy: load heading_pitch_V model from {os.path.join(os.getcwd(),"/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/5v5_lczh/AeroPlanax_multi_combat_5v5/envs/models/RNN新策略/PPO+RNN(actor与critic均加了layer_norm且改进训练过程（暖启动+防梯度消失+reward_clip)/seed42/heading_pitch_V_discrete_rnn_2025-08-29-15-55/checkpoints/checkpoint_epoch_600")}')
config = {
    "SEED": 42,
    "LR": 3e-4,
    "NUM_ENVS": 1,
    ...
    "LOADDIR": os.path.join(os.getcwd(),"/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/5v5_lczh/AeroPlanax_multi_combat_5v5/envs/models/RNN新策略/PPO+RNN(actor与critic均加了layer_norm且改进训练过程（暖启动+防梯度消失+reward_clip)/seed42/heading_pitch_V_discrete_rnn_2025-08-29-15-55/checkpoints/checkpoint_epoch_600") # 基于RNN训练出来的baseline
}
...
##############################################################################################################################
    ...
    def train_callback(self, metric: chex.Array, writer:tensorboardX.SummaryWriter, train_mode:bool):
        ...
        if hasattr(self, '_curriculum_params') and self._curriculum_params.enable_curriculum:
            ...
            # 记录到tensorboard
            writer.add_scalar('curriculum/progress', curriculum_progress, env_steps)
            writer.add_scalar('curriculum/enemy_damage', current_enemy_damage, env_steps)
            writer.add_scalar('curriculum/maneuver_factor', current_maneuver_factor, env_steps)
