...
print(f'combat_hierarchy policy: load heading_pitch_V model from {os.path.join(os.getcwd(),"/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/2v2_lczh_mine/AeroPlanax_multi_combat_2v2/envs/models/LSTM新策略/PPO+LSTM(仅actor加了layer_norm且改进训练过程（暖启动+防梯度消失+reward_clip)（不带中间奖励并且unreach_heading_pitch_V.py里面mask1只由时间决定（宽松版）训练出来的）/seed42/heading_pitch_V_discrete_lstm_2025-08-29-15-01/checkpoints/checkpoint_epoch_600")}')
config = {
    "SEED": 42,
    "LR": 3e-4,
    "NUM_ENVS": 1,
    ...
    "LOADDIR": os.path.join(os.getcwd(),"/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/2v2_lczh_mine/AeroPlanax_multi_combat_2v2/envs/models/LSTM新策略/PPO+LSTM(仅actor加了layer_norm且改进训练过程（暖启动+防梯度消失+reward_clip)（不带中间奖励并且unreach_heading_pitch_V.py里面mask1只由时间决定（宽松版）训练出来的）/seed42/heading_pitch_V_discrete_lstm_2025-08-29-15-01/checkpoints/checkpoint_epoch_600")
}
