# Planax/train_waypoint_vertical_loop.py
import os
...
from envs.aeroplanax_waypoint_vertical_loop_train import VerticalLoopEnv, VerticalLoopParams
...
def make_train(config):
    ...
    # 新增：筋斗课程参数
    cfg.setdefault("LOOP_PROB_START", 0.0)  # 初始筋斗概率
    cfg.setdefault("LOOP_PROB_END", 1.0)    # 最终筋斗概率
    cfg.setdefault("UPDATES_TO_FULL_LOOP", 2000)  # 线性上升到满概率的 update 数

    # 默认环境参数（课程中会动态调整）
    env_params = VerticalLoopParams(
        max_steps=3000,
        sim_freq=50,
        agent_interaction_steps=10,

        loop_radius=6000.0,
        loop_points_per_circle=240,
        loop_forward_north=0.0,
        loop_target_vt=230.0,
        loop_phase0_deg=180.0,
        loop_direction=-1,
        success_after_loops = 1,              # 新增：完成多少圈判定成功

        lookahead_base_pts=8,
        lookahead_gain_pts=24,
        pitch_limit_deg_up=55.0,
        pitch_limit_deg_down=70.0,

        reach_radius_init=800.0,
    ...
    env = VerticalLoopEnv(env_params)
    env = LogWrapper(env)
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, _):
            ...
            # ====== 统计 + 日志 ======
            loss_mean = jax.tree_util.tree_map(lambda x: jnp.nan_to_num(x.mean(), nan=0.0, posinf=0.0, neginf=0.0), loss_info)
            ratio_0 = jnp.nan_to_num(loss_info["ratio"].at[0, 0].get().mean(), nan=0.0, posinf=0.0, neginf=0.0)
            ...
            # ====== reached_count 聚合（新增） ======
            rc = traj_batch.info.get("reached_count", jnp.zeros_like(traj_batch.valid_action))
            rc = rc.astype(jnp.float32)
            metric["reached_count_mean"] = (rc * mask).sum() / denom         #  本次 rollout 全体平均
            metric["reached_count_last"] = rc[-1].mean()                      # 最后一个时间步的均值
            metric["reached_count_max"]  = rc.max()                           # 本次 rollout 内的最大值
            # ---------------------------------------------------------------
            ...
            # 重置环境（固定参数）
            reset_rng = jax.random.split(rng, cfg["NUM_ENVS"])
            obsv, env_state = jax.vmap(env.reset, in_axes=(0,))(reset_rng)
            # 重置后的观测需要 batchify；done 置零
            obsv = batchify(obsv, env.agents, cfg["NUM_ENVS"], cfg["NUM_ACTORS"])
            done0 = jnp.zeros((cfg["NUM_ENVS"] * cfg["NUM_ACTORS"]), dtype=bool)
            hstate = jax.lax.stop_gradient(jnp.zeros_like(hstate))
            ...
            if cfg.get("DEBUG"):
                def callback(m):
                    ...
                    # 评估曲线（可选存在）
                    if "returned_episode_returns" in m and "returned_episode" in m:
                        try:
                            ret_arr  = jnp.asarray(m["returned_episode_returns"][m["returned_episode"]])
                            elen_arr = jnp.asarray(m["returned_episode_lengths"][m["returned_episode"]])
                            ret  = float(jnp.nan_to_num(ret_arr.mean(),  nan=0.0, posinf=0.0, neginf=0.0))
                            elen = float(jnp.nan_to_num(elen_arr.mean(), nan=0.0, posinf=0.0, neginf=0.0))
                            writer.add_scalar('eval/episodic_return', ret, env_steps)
                            writer.add_scalar('eval/episodic_length', elen, env_steps)
                        except Exception:
                            ret, elen = 0.0, 0.0
                    else:
                        ret, elen = 0.0, 0.0
                    ...
                    # reached_count（新增）
                    if "reached_count_mean" in m:
                        writer.add_scalar('loop/reached_count_mean', float(m["reached_count_mean"]), env_steps)
                    if "reached_count_last" in m:
                        writer.add_scalar('loop/reached_count_last', float(m["reached_count_last"]), env_steps)
                    if "reached_count_max" in m:
                        writer.add_scalar('loop/reached_count_max',  float(m["reached_count_max"]),  env_steps)

                    # 奖励裁剪（存在才记）
                    for key in [
                        'clipped_altitude_reward_count',
                        'clipped_heading_pitch_V_reward_count',
                        'clipped_any_reward_count',
                        'clipped_altitude_reward_count_rate',
                        'clipped_heading_pitch_V_reward_count_rate',
                        'clipped_any_reward_count_rate',
                    ]:
                        if key in m:
                            writer.add_scalar(f'reward_clip/{key}', float(m[key]), env_steps)

                    # 终端打印：增加 reached_count_last
                    rc_last = float(m.get("reached_count_last", jnp.array(0.0)))
                    print("EnvStep={:<10} EpisodeLength={:<6.2f} Return={:<7.2f} Reached(last)={:.2f}".format(
                        env_steps, elen, ret, rc_last
                    ))
                jax.experimental.io_callback(callback, None, metric)

            runner_state = (train_state, env_state, obsv, done0, hstate, rng)
...
config = {
    "GROUP": "vertical_loop_ppo_rnn",
    "SEED": 42,
    "FOR_LOOP_EPOCHS": 1,
    "LR": 3e-4,
    "NUM_ENVS": 1000,  # 并行环境数（调大加速）
    "NUM_ACTORS": 1,
    "NUM_STEPS": 1000,
    "TOTAL_TIMESTEPS": 1e8,
    "FC_DIM_SIZE": 128,
    ...
    "OUTPUTDIR": "results/" + "vertical_loop_ppo_rnn" + "_" + str_date_time,
    "LOGDIR": "results/" + "vertical_loop_ppo_rnn" + "_" + str_date_time + "/logs",
    "SAVEDIR": "results/" + "vertical_loop_ppo_rnn" + "_" + str_date_time + "/checkpoints",
    "LOADDIR": "/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/2v2_lczh_mine/AeroPlanax_multi_combat_2v2/envs/models/RNN新策略/PPO+RNN(actor与critic均加了layer_norm且改进训练过程（暖启动+防梯度消失+reward_clip)/seed10/heading_pitch_V_discrete_rnn_2025-08-29-12-46/checkpoints/checkpoint_epoch_600"  # 你的 baseline checkpoint 目录
}
...
wandb.init(
    ...
    name=config['GROUP'],
    group=config['GROUP'],
    notes='PPO RNN for vertical loop waypoint task',
    reinit=True,
)
