# 只保留了只保留‘成功率≥80%且试次≥60’的升桶，移除动压/能量门和冗余 info/metric”
...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, _):
            ...
            # 计算最后一步的众数桶（0..8）与占比 —— 先拍平为 1D int 向量
            bin_last = traj_batch.info["pitch_current_bin"][last]
            bin_last = jnp.nan_to_num(bin_last, nan=0.0).astype(jnp.int32).reshape(-1)  # 保证1D
            bin_last = jnp.clip(bin_last, 0, 8)                                         # 安全夹取

            hist = jnp.bincount(bin_last, length=9)                                     # (9,)
            bin_idx_mode  = jnp.argmax(hist)
            bin_mode_share= jnp.max(hist) / (bin_last.size + 1e-8)

            # metric 区域（你已有 bin_idx_mode）
            t_last      = traj_batch.info["curr_t_cur"][-1].reshape(-1)                 # (B,)
            bin_last    = traj_batch.info["pitch_current_bin"][-1].astype(jnp.int32).reshape(-1)

            # 这里不要从 metric 里读，直接用局部 bin_idx_mode
            bin_idx_mode_i32 = jnp.asarray(bin_idx_mode, jnp.int32)
            mask_mode   = (bin_last == bin_idx_mode_i32)

            t_cur_mode  = jnp.where(mask_mode, t_last, 0.0)
            denom       = jnp.maximum(jnp.sum(mask_mode), 1)
            metric["curr/t_cur_mode_mean"] = jnp.sum(t_cur_mode) / denom

            # curr/t_cur_p50: “最后一步所有并行环境的 t_cur 的中位数”。≥60 表示至少一半环境在当前桶的试次已达 60。
            # curr/t_cur_p90: “最后一步所有并行环境的 t_cur 的 90 分位”。≥60 表示至少 90% 环境已达 60。
            # 用它们判断“多数环境是否真到线”，比均值更稳，不会被“刚升桶的 env（t_cur≈0）”拉低太多。
            metric["curr/t_cur_p50"] = jnp.percentile(t_last, 50.0)
            metric["curr/t_cur_p90"] = jnp.percentile(t_last, 90.0)

            metric["curr/bin_idx_mode"]     = bin_idx_mode.astype(jnp.float32)
            metric["curr/bin_mode_share"]   = bin_mode_share.astype(jnp.float32)
            metric["curr/bin_lo_deg_mode"]  = bin_idx_mode.astype(jnp.float32) * env_params.pitch_bin_deg
            metric["curr/bin_hi_deg_mode"]  = (bin_idx_mode.astype(jnp.float32) + 1.0) * env_params.pitch_bin_deg
            ...
            if cfg.get("DEBUG"):
                def callback(m):
                    ...
                    writer.add_scalar('curr/t_cur_mode_mean',      float(m["curr/t_cur_mode_mean"]),      env_steps)
                    writer.add_scalar('curr/t_cur_p50',  float(m["curr/t_cur_p50"]),  env_steps)
                    writer.add_scalar('curr/t_cur_p90',  float(m["curr/t_cur_p90"]),  env_steps)
                    ...
                    print(
                        (
                            ...
                            "Curr: bin_mean[{:>2.0f},{:>2.0f}]  bin_mode[{:>2.0f},{:>2.0f}] (idx_mode={:.0f}, share={:.1%}) cur_success_rate={:.1%} cur_bin_idx={:.0f} history_max_bin_idx_mean={:.0f} t_cur={:.0f} t_cur_mode_mean={:.0f} t_cur_p50={:.0f} t_cur_p90={:.0f}\n"
                            "Alt: Alt mean={:.2f}km\n"
                        ).format(
                            env_steps,
                            ...
                            float(m["curr/bin_lo_deg_mode"]), float(m["curr/bin_hi_deg_mode"]),
                            float(m["curr/bin_idx_mode"]), float(m["curr/bin_mode_share"]),
                            float(m["curr/rate_cur"]),
                            float(m["curr/bin_idx"]),
                            float(m["curr/max_bin"]),
                            float(m["curr/t_cur"]),
                            float(m["curr/t_cur_mode_mean"]),
                            float(m["curr/t_cur_p50"]),
                            float(m["curr/t_cur_p90"]),
                            float(m["alt/mean_km"]),
                        ),
                        flush=True,
                    )
