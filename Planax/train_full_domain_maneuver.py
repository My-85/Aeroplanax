...
def make_train(config):
    ...
    cfg.setdefault("ENT_COEF_MIN", 1e-3)       # raised from 5e-4
    cfg.setdefault("ENT_COEF_MAX", 5e-2)        # raised from 2e-2
    cfg.setdefault("ENT_ADJ_RATE", 1.05)
    cfg.setdefault("LR_DECAY", 0.999)
    cfg.setdefault("MIN_LR_MULT", 0.2)

    cfg.setdefault("WARMUP_UPDATES",     2000)   # raised from 1500
...
config = {
    ...
    "OUTPUTDIR": "results/" + "full_domain_maneuver" + "_" + str_date_time,
    "LOGDIR": "results/" + "full_domain_maneuver" + "_" + str_date_time + "/logs",
    "SAVEDIR": "results/" + "full_domain_maneuver" + "_" + str_date_time + "/checkpoints",
}
...
wandb.init(
    ...
    name=config['GROUP'],
    group=config['GROUP'],
    notes='Full-domain maneuver training: arbitrary 3D attitude control with curriculum',
    reinit=True,
)
