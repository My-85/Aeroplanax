...
def _controller_obs(state: fighterplane.FighterPlaneState, target_pitch, target_heading, target_vt):
    # 保证 target_* 都是 (B,) 形状，兼容你内部 baseline 分支传入的标量
    B = state.vt.shape[0]
    target_heading = jnp.broadcast_to(jnp.asarray(target_heading), (B,))
    target_pitch   = jnp.broadcast_to(jnp.asarray(target_pitch),   (B,))
    target_vt      = jnp.broadcast_to(jnp.asarray(target_vt),      (B,))

    # 当前姿态 q_BN (NED->Body)
    q_curr = jnp.stack([
        jnp.nan_to_num(state.q0, nan=0.0),
        jnp.nan_to_num(state.q1, nan=0.0),
        jnp.nan_to_num(state.q2, nan=0.0),
        jnp.nan_to_num(state.q3, nan=0.0),
    ], axis=1)  # (B,4)
    q_curr = jax.vmap(_quat_normalize)(q_curr)

    # 误差四元数（w>=0），取向量部 3 维
    def _err_row(q_row, yh, ph):
        return _quat_err_nb(q_row, yh, ph, 0.0)  # roll target 恒为 0
    q_err_batch = jax.vmap(_err_row, in_axes=(0, 0, 0))(q_curr, target_heading, target_pitch)  # (B,4)
    qv = jnp.clip(q_err_batch[:, 1:4], -1.0, 1.0)  # (B,3)

    # 机体系下目标方向 v_b：先用 (heading,pitch) 构 NED 单位向量，再旋到 Body
...
class AeroPlanaxWaypointEnv(AeroPlanaxEnv[WaypointTaskState, WaypointTaskParams]):
    ...
    def _decode_actions(
        ...
        def use_internal(_):
            ...
            if self.default_params.use_s_curve:
    ...
    def _init_state(self, key: jax.Array, params: WaypointTaskParams) -> WaypointTaskState:
        ...
        q1 = jnp.array([0.0]); q2 = jnp.array([0.0])
        ...
        s = s.replace(plane_state=s.plane_state.replace(yaw=yaw, vt=vt0, q0=q0, q1=q1, q2=q2, q3=q3, altitude=alt0))
        ...
        if params.use_s_curve:
            ...
            difficulty = jnp.array(idx0)  # 用作“当前步数/奇偶”的计数器
        else:
            # 原随机首航点逻辑...
    ...
    def _step_task(
        ...
        if params.s_altitude_lock:
            desired_pitch = 0.0
            dist3d = hdist  # 锁高模式下，仅用平面距离做达成判据
        ...
        def on_reach(_):
            ...
            if params.use_s_curve:
                # # 计数器 +1 用作奇偶位（0: +A, 1: -A, 2: +A, ...）
                # new_diff = state.difficulty + 1
                # sign = jnp.where((new_diff % 2) == 0, 1.0, -1.0)
                # wp = jnp.array([
                #     pn + params.s_step_north,                 # 沿北向推进
                #     pe + sign * params.s_amplitude,           # 左右交替
                #     state.plane_state.altitude[0],            # 恒定高度 = 当前高度
                # ])
                # # 航点更密集 → 建议半径不再继续衰减（或者轻微衰减）
                ...
                new_idx = state.difficulty + 1
                dn = params.s_half_period_north / params.s_points_per_half
                n_next = state.s_origin_n + new_idx * dn
                e_next = state.s_origin_e + params.s_amplitude * jnp.sin(
                    jnp.pi * (n_next - state.s_origin_n) / params.s_half_period_north
                )
                wp = jnp.array([n_next, e_next, state.plane_state.altitude[0]])
    ...
    def _get_obs(
        ...
        obs_mat = _controller_obs(state.plane_state, desired_pitch, desired_heading, target_vt)  # (16,B)
