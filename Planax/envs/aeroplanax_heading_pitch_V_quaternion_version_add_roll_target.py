...
class Heading_Pitch_V_TaskParams(EnvParams):
    ...
    max_roll_increment: float = jnp.pi/3   # 每次任务切换滚转最大变化（≈60°）
    safe_roll_limit: float = jnp.radians(75.0)  # 绝对滚转安全范围（±75°）
...
class AeroPlanaxHeading_Pitch_V_Env(AeroPlanaxEnv[Heading_Pitch_V_TaskState, Heading_Pitch_V_TaskParams]):
    def __init__(self, env_params: Optional[Heading_Pitch_V_TaskParams] = None):
        ...
        self.reward_functions = [
            functools.partial(heading_pitch_V_reward_fn_add_roll_target, reward_scale=1.0),
    ...
    def _init_state(
        ...
        # 初始把目标滚转设为当前 roll（更自然，也更稳）
        target_roll0 = state.plane_state.roll
        extra_state = jnp.stack([
            jnp.zeros((self.num_agents,)),  # target_heading 占位（后面 _reset_task 会写）
            jnp.zeros((self.num_agents,)),  # target_pitch
            jnp.zeros((self.num_agents,)),  # target_vt
            target_roll0,                   # target_roll
        ], axis=0)
        state = Heading_Pitch_V_TaskState.create(state, extra_state=extra_state)
        return state
    ...
    def _reset_task(
        ...
        state = state.replace(
            ...
            target_vt=vt,
            target_roll=state.plane_state.roll,   # <-- 新增
    ...
    def _step_task(
        ...
        key_heading, key_pitch, key_vt_increment, key_roll = jax.random.split(key, 4)
        ...
        safe_pitch_min = jnp.radians(-85.0)  # -45度
        safe_pitch_max = jnp.radians(85.0)   # +45度
        ...
        # 滚转变化量（受安全范围限制）
        max_roll_inc = params.max_roll_increment
        delta_roll = jax.random.uniform(
            key_roll, shape=(self.num_agents,),
            minval=-max_roll_inc, maxval=max_roll_inc
        )

        # 新的滚转目标：在当前 roll 基础上变化，并裁到安全上下限
        new_roll = state.plane_state.roll + delta_roll * delta
        safe_roll = params.safe_roll_limit
        new_roll = jnp.clip(new_roll, -safe_roll, safe_roll)

        # 真正的增量（裁剪后）
        delta_roll = new_roll - state.plane_state.roll
