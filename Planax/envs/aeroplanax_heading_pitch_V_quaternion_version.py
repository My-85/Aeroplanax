...
class Heading_Pitch_V_TaskParams(EnvParams):
    ...
    # min_vt: float = 50.0     # [修改] 稍微降低下限，让它也见过筋斗顶点的低速状态
    min_vt: float = 100.0     # [修改] 稍微降低下限，让它也见过筋斗顶点的低速状态
...
class AeroPlanaxHeading_Pitch_V_Env(AeroPlanaxEnv[Heading_Pitch_V_TaskState, Heading_Pitch_V_TaskParams]):
    ...
    def _init_state(
        ...
        # 在 _init_state 和 _reset_task 中
        # 原代码：
        # q0 = jnp.cos(half_heading) ... (只考虑了 yaw，pitch默认为0)
    ...
    def _step_task(
        ...
        def _theta_row(q_row, yh, ph, vt, vt_tgt):
            q_err = _quat_err_nb(q_row, yh, ph, 0.0)
        ...
        hpv_reward_raw = jax.vmap(_theta_row, in_axes=(0,0,0,0,0))(
            q_curr, state.target_heading, state.target_pitch, state.plane_state.vt, state.target_vt
        )
    ...
    def _get_obs(
        ...
        def _err_row(q_row, yh, ph):
            q_err = _quat_err_nb(q_row, yh, ph, 0.0)
            return q_err
        q_err_batch = jax.vmap(_err_row, in_axes=(0,0,0))(q_curr, yaw_t, pitch_t)  # (B,4)
