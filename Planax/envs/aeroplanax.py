...
# Removed unused imports: canardplane, uav (placeholder modules never invoked).
...
class AeroPlanaxEnv(Generic[TEnvState, TEnvParams]):
    ...
    def __init__(
        ...
        self.use_artillery = getattr(env_params, "use_artillery", False)
        ...
        self.is_potential: List[bool] = []   # 新增（与旧版对齐）

        self.create_records = False
        # 仅用于一次性输出预计算航点到 Tacview
        self._wp_dumped = False  # 仅第一次渲染时把整圈航点写入
        # === Tacview Object ID Ranges (avoid collisions) ===
        self.ID_BASE_ALLY     = 100     # 友机：100 + i
        self.ID_BASE_ENEMY    = 11000     # 敌机：11000 + i
        self.ID_BASE_MISSILE  = 12000     # 导弹：12000 + i
        self.ID_BASE_TARGET   = 1000     # Target_* 动态标记：1000 + i
        self.ID_BASE_WP       = 500000    # 静态航点：500000 + k
        self.ID_POLYLINE_WP   = 510000    # 航点折线：510000

        # 新增：整圈航点（ENU），形状 [K,3] = [north, east, altitude]
        loop_wps: jnp.ndarray = jnp.zeros((0, 3), dtype=jnp.float32)
    ...
    def _init_state(
        ...
        env_state = EnvState(
            ...
            pre_rewards=jnp.zeros((len(self.reward_functions), self.num_agents)),  # ← 新增
            done=False,
            success=False,
            time=0
        )
    ...
    def render(
        ...
        if not self.create_records:
            ...
            self._wp_dumped = False  # 新文件时重置
        # 紧挨着 with open(self.filename, mode='a', ...) 之前或之后添加
        def _pick(x, i=None):
            a = jnp.asarray(x)
            if a.ndim == 0:
                return float(a)
            if a.ndim == 1:
                return float(a[i if i is not None else 0])
            if a.ndim == 2:
                return float(a[0, i if i is not None else 0])
        ...
        with open(self.filename, mode='a', encoding='utf-8') as f:
            ...
            for i in range(self.num_allies):
                ...
                # log_msg += f"Name=F16,"
                # log_msg += f"Color=Red"
                log_msg = f"{self.ID_BASE_ALLY + i},T={lon}|{lat}|{alt}|{roll}|{pitch}|{yaw},Type=Air+FixedWing,Name=F16,Color=Red"
                ...
                if hasattr(state, 'target_heading') and hasattr(state, 'target_pitch'):
                    ...
                    target_msg  = (
                                    f"{self.ID_BASE_TARGET + i},"
                                    f"T={target_lon}|{target_lat}|{target_alt}|0|{target_pitch * 180 / jnp.pi}|{target_heading * 180 / jnp.pi},"
                                    f"Name=Target_{i},Color=Yellow,Type=Marker"
                                )
            ...
            for i in range(self.num_allies, self.num_agents):
                ...
                log_msg = f"{self.ID_BASE_ALLY + i},T={lon}|{lat}|{alt}|{roll}|{pitch}|{yaw},Name=F16,Color=Red"
        
                if log_msg is not None:
                    f.write(log_msg + "\n")
            for i in range(self.num_missiles):
                npos = _pick(state.missile_state.north, i)
                epos = _pick(state.missile_state.east, i)
                alt = _pick(state.missile_state.altitude, i)
                roll = _pick(state.missile_state.roll, i) * 180 / jnp.pi
                pitch = _pick(state.missile_state.pitch, i) * 180 / jnp.pi
                yaw = _pick(state.missile_state.yaw, i) * 180 / jnp.pi
                lat, lon, alt = enu_to_geodetic(epos, npos, alt, 0, 0, 0)
                log_msg = f"{100 + self.num_agents + i},T={lon}|{lat}|{alt}|{roll}|{pitch}|{yaw},"
                log_msg += f"Type=Air+Missile,Name=AIM-9L,"
                log_msg += f"Color=Blue"
                if log_msg is not None:
                    f.write(log_msg + "\n")

            # === 一次性写出预计算整圈航点（用 Marker，且不带显式时间前缀） ===
            try:
                # --- 替换：一次性写出整圈航点（2.2 规范的静态航点），并删除 Polyline 输出 ---
                wps = jnp.asarray(getattr(state, "loop_wps", None))
                if (not self._wp_dumped) and (wps is not None) and (wps.ndim == 2) and (wps.shape[0] > 0):
                    for k in range(int(wps.shape[0])):
                        n, e, a = float(wps[k,0]), float(wps[k,1]), float(wps[k,2])
                        lat, lon, alt = enu_to_geodetic(e, n, a, 0, 0, 0)
                        oid_mk = self.ID_BASE_WP + k
                        f.write(
                            f"{oid_mk},"
                            f"Type=Navaid+Static+Waypoint,Name=WP_{k},Label={k+1},Color=Yellow,"
                            f"T={lon}|{lat}|{alt}|0|0|0\n"
                        )
                    self._wp_dumped = True
