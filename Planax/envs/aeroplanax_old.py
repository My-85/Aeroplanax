...
class AeroPlanaxEnv(Generic[TEnvState, TEnvParams]):
    ...
    def __init__(
        ...
        # 初始化阶段标记缓存（用于写切换标记）
        self._last_vert_flag = [None] * int(self.num_allies)
        self._last_vert_up_flag = [None] * int(self.num_allies)
        self._switch_marker_counter = 12000
        self._ray_initialized = [False] * int(self.num_allies)
        self._mast_initialized = [False] * int(self.num_allies)
    ...
    def step(
        ...
        def step_sim_fn(state_st, _):
            ...
            state_st = state_st.replace(
                ...
                control_state=action,          # <<< 新增：把本步舵量存起来，渲染/平滑都能用
            )
    ...
    def render(
        ...
        """
        同时兼容：
        - batched: (B, num_agents) 形状（baseline 训练/评测）
        - unbatched: (num_agents,) 形状（单环境脚本）
        """

        """Renders the environment."""
        Path(logdir).mkdir(parents=True, exist_ok=True)
        self.create_records = getattr(self, "create_records", False)
        if bool(dones.get("__all__", False)):
            self.create_records = False
        if not self.create_records:
            self.filename = str(Path(logdir) / (datetime.now().strftime('%Y-%m-%d-%H-%M-%S.%f') + '.txt.acmi'))
            ...
            with open(self.filename, mode="a", encoding="utf-8") as f:
                for i in range(self.num_allies):
                    f.write(f"0,{9000 + i},Type=ReferencePoint,Name=WP_{i},Color=Yellow\n")
                for i in range(self.num_allies):
                    f.write(f"0,{1000 + i},Type=ReferencePoint,Name=Target_{i},Color=Yellow\n")  # 如果你保留“目标指示器”


        # 取数助手：同时兼容有/无 batch 维
        def _pick(x, i=None):
            a = jnp.asarray(x)
            if a.ndim == 0:
                return float(a) 
            if a.ndim == 1:           # (num_agents,)
                return float(a[i] if i is not None else a[0])
            if a.ndim == 2:           # (batch, num_agents)
                return float(a[0, i] if i is not None else a[0, 0])
            # 兜底
            a = a.reshape(-1, a.shape[-1] if i is not None else -1)
            return float(a[0, i] if i is not None else a[0, 0])

        # 时间戳
        ts = _pick(state.time) * self.agent_interaction_steps / params.sim_freq
        ...
        with open(self.filename, mode='a', encoding='utf-8') as f:
            ...
            for i in range(self.num_allies):
                ...
                npos = _pick(state.plane_state.north,    i)
                epos = _pick(state.plane_state.east,     i)
                alt  = _pick(state.plane_state.altitude, i)
                roll = _pick(state.plane_state.roll,     i) * 180 / jnp.pi
                pitch= _pick(state.plane_state.pitch,    i) * 180 / jnp.pi
                yaw  = _pick(state.plane_state.yaw,      i) * 180 / jnp.pi
                lat, lon, alt_geo = enu_to_geodetic(epos, npos, alt, 0, 0, 0)
                # f.write(f"{100+i},T={lon}|{lat}|{alt_geo}|{roll}|{pitch}|{yaw},Name=F16,Color=Red\n")

                # ===== 阶段着色：NORMAL=0xFF8800(橙), VERT_UP=0xFF00FF(洋红), VERT_DOWN=0x00FFFF(青) =====
                # 1) 先读环境 flag；2) flag 不可用时，用阈值回退(|target_pitch|>=70°)
                is_vert = False
                is_up = False
                if hasattr(state, 'is_vertical_target'):
                    try:
                        is_vert = bool(int(_pick(state.is_vertical_target, i)))
                    except Exception:
                        is_vert = False
                if hasattr(state, 'is_vertical_up_target'):
                    try:
                        is_up = bool(int(_pick(state.is_vertical_up_target, i)))
                    except Exception:
                        is_up = False
                if not is_vert:
                    theta = _pick(state.target_pitch if hasattr(state, 'target_pitch') else state.plane_state.pitch, i)
                    theta_deg = float(theta * 180.0 / jnp.pi)
                    if abs(theta_deg) >= 70.0:
                        is_vert = True
                        is_up = theta_deg > 0.0

                COLOR_NORM_NAME = "White"   # 常规
                COLOR_VUP_NAME  = "Yellow"  # 竖直上
                COLOR_VDN_NAME  = "Green"   # 竖直下
                color_hex = COLOR_VUP_NAME if (is_vert and is_up) else (COLOR_VDN_NAME if is_vert else COLOR_NORM_NAME)
                name_suffix = "(VUP)" if (is_vert and is_up) else ("(VDOWN)" if is_vert else "(NORM)")

                # 飞机本体颜色固定为 Red，避免被 Tacview 降级为灰
                f.write(f"{100+i},T={lon}|{lat}|{alt_geo}|{roll}|{pitch}|{yaw},Name=F16{name_suffix},Color=Red\n")

                # 在阶段切换点，打一个 ReferencePoint 标记
                if hasattr(self, '_last_vert_flag'):
                    prev_flag = self._last_vert_flag[i]
                    prev_up   = self._last_vert_up_flag[i]
                    if (prev_flag is None) or (prev_flag != is_vert) or (prev_up != is_up):
                        phase = "NORMAL" if not is_vert else ("VERT_UP" if is_up else "VERT_DOWN")
                        mid = int(getattr(self, "_switch_marker_counter", 12000))
                        f.write(f"{mid},Type=ReferencePoint,Name={phase},Color={color_hex}\n")
                        f.write(f"{mid},T={lon}|{lat}|{alt_geo}|0|0|0\n")
                        self._switch_marker_counter = mid + 1
                        self._last_vert_flag[i] = is_vert
                        self._last_vert_up_flag[i] = is_up
                ...
                if hasattr(state, 'target_heading') and hasattr(state, 'target_pitch'):
                    ...
                    target_heading = _pick(state.target_heading, i)
                    target_pitch   = _pick(state.target_pitch, i)
                    dy = target_distance * jnp.cos(target_pitch) * jnp.cos(target_heading)
                    dx = target_distance * jnp.cos(target_pitch) * jnp.sin(target_heading)
                    dz = target_distance * jnp.sin(target_pitch)
                    target_npos = npos + dy
                    ...
                    target_msg = f"{1000 + i},T={target_lon}|{target_lat}|{target_alt}|0|{target_pitch * 180 / jnp.pi}|{target_heading * 180 / jnp.pi},"
                    target_msg += f"Name=Target_{i},"
                    target_msg += f"Color={color_hex},"
                    target_msg += f"Type=Marker"
                    f.write(target_msg + "\n")

                    # 相位射线（Polyline）：首次声明，随后每帧仅更新 T
                    line_id = 2000 + i
                    if not self._ray_initialized[i]:
                        f.write(f"{line_id},Type=Polyline,Name=Ray_{i}{name_suffix},Color={color_name},LineWidth=4\n")
                        self._ray_initialized[i] = True
                    f.write(f"{line_id},T={target_lon}|{target_lat}|{target_alt} {lon}|{lat}|{alt_geo}\n")

                    # 竖直段相位旗杆（Polyline）：从机体竖直上/下 800m
                    if is_vert:
                        mast_id = 3000 + i
                        if not self._mast_initialized[i]:
                            f.write(f"{mast_id},Type=Polyline,Name=Phase_{i}{name_suffix},Color={color_name},LineWidth=4\n")
                            self._mast_initialized[i] = True
                        mast_alt2 = float(alt_geo + (800.0 if is_up else -800.0))
                        f.write(f"{mast_id},T={lon}|{lat}|{alt_geo} {lon}|{lat}|{mast_alt2}\n")

                # ============ Waypoint 任务：目标点 + 由本机到航点的“目标轨迹（点列）” ============
                if hasattr(state, "waypoint"):
                    wp = jnp.asarray(state.waypoint)
                    if wp.ndim == 2:
                        wp_n, wp_e, wp_alt = float(wp[0, 0]), float(wp[0, 1]), float(wp[0, 2])
                    else:
                        wp_n, wp_e, wp_alt = float(wp[0]), float(wp[1]), float(wp[2])
                    wlat, wlon, walt = enu_to_geodetic(wp_e, wp_n, wp_alt, 0, 0, 0)
                    # 只用一个固定 ID：9000+i，类型在文件头已创建；这里仅更新位置
                    f.write(
                        f"{9000 + i},T={wlon}|{wlat}|{walt}|0|0|0\n"
                    )
                # =============================  Waypoint 绘制结束  =============================
