...
def _controller_obs(state: fighterplane.FighterPlaneState, target_pitch, target_heading, target_vt):
    # 目标差
    norm_delta_heading = _wrap_pi(state.yaw - target_heading)
    norm_delta_pitch   = _wrap_pi(state.pitch - target_pitch)
    norm_delta_vt      = (state.vt - target_vt) / 340.0
    norm_altitude      = state.altitude / 5000.0
    norm_vt            = state.vt / 340.0

    # 三角/角速度
    roll_sin, roll_cos   = jnp.sin(state.roll),  jnp.cos(state.roll)
    pitch_sin, pitch_cos = jnp.sin(state.pitch), jnp.cos(state.pitch)
    alpha_sin, alpha_cos = jnp.sin(state.alpha), jnp.cos(state.alpha)
    beta_sin,  beta_cos  = jnp.sin(state.beta),  jnp.cos(state.beta)
    P, Q, R = state.P, state.Q, state.R

    # 大气与能量
    alt_ft = state.altitude / 0.3048
    vt_ft  = jnp.clip(state.vt / 0.3048, 0.1, 1e6)
    mach, qbar, _ = atmos(alt_ft, vt_ft)
    # 参考动压：中位高度 + 参考速度 360m/s
    alt_mid_ft = ((10000.0 + 20000.0) * 0.5) / 0.3048
    vt_ref_ft  = 360.0 / 0.3048
    _, qbar_ref, _ = atmos(alt_mid_ft, vt_ref_ft)
    qbar_norm = jnp.clip(qbar / (qbar_ref + 1e-6), 0.0, 2.0)

    g = 9.81
    spec_energy = g * state.altitude + 0.5 * jnp.clip(state.vt, 0.0, 1e4) ** 2
    e_ref = g * 20000.0 + 0.5 * (360.0 ** 2)
    energy_norm = jnp.clip(spec_energy / (e_ref + 1e-6), 0.0, 1.5)

    # 航迹角
    vh = jnp.sqrt(jnp.maximum(state.vel_x * state.vel_x + state.vel_y * state.vel_y, 1e-6))
    gamma = jnp.arctan2(-state.vel_z, vh)
    ...
    obs = jnp.vstack((
        ...
        P, Q, R,
        state.az, mach, qbar_norm, energy_norm, gamma_sin, gamma_cos
    ))
    return obs
...
class AeroPlanaxWaypointEnv(AeroPlanaxEnv[WaypointTaskState, WaypointTaskParams]):
    ...
    def _get_individual_obs_space(self, i) -> spaces.Space:
        ...
        return spaces.Box(-jnp.inf, jnp.inf, (22,), dtype=jnp.float32)
    ...
    def _init_controller(self, params: WaypointTaskParams):
        ...
        init_x = (
            jnp.zeros((1, self.cfg["NUM_ENVS"] * self.cfg["NUM_ACTORS"], 22)),
    ...
    def _decode_actions(
        ...
        def use_internal(_):
            ...
            hstate, pi, _value, _pred = self.controller.apply(self.controller_params, state.hstate, ac_in)
