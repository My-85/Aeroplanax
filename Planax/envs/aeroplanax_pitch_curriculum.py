...
class Pitch_Curriculum_TaskParams(EnvParams):
    ...
    curriculum_min_trials: int = 20             # 当前桶最小尝试数
...
class AeroPlanaxPitchCurriculumEnv(AeroPlanaxEnv[Pitch_Curriculum_TaskState, Pitch_Curriculum_TaskParams]):
    ...
    def _reset_task(self, key: chex.PRNGKey, state: Pitch_Curriculum_TaskState, params: Pitch_Curriculum_TaskParams) -> Pitch_Curriculum_TaskState:
        ...
        # 你在 _reset_task 里对 bin_trials/bin_successes 做了 0.995 的衰减（并跨 episode 平均），导致每次 reset 都把历史试次“磨小”，t_cur 很难堆到 60，自然永远升不了第一桶。
        # 解决：把衰减取消（或先设 1.0 验证），让 bin_trials/bin_successes 跨 episode 累积不被磨掉。这样每次 attempt 结束都会稳步把 t_cur 加 1，直到达到 60 才会在成功步触发升桶。
        decay = 1.0  # 或 1.0 先验证
    ...
    def _step_task(
        ...
        if params.render_no_switch:
            ...
            info["curr_t_cur"]         = t_cur.astype(jnp.float32)
        ...
        # 切换目标/刷新时间时，允许“因 promote”强制切换，但不额外+1试次
        do_switch_for_target = do_switch | promote

        # 下面 state.replace/采样新目标/last_check_time 等，把 do_switch 全部替换为 do_switch_for_target
        new_tgt_heading = jnp.where(do_switch_for_target, sampled_heading, state.target_heading)
        new_tgt_pitch   = jnp.where(do_switch_for_target, sampled_pitch,  state.target_pitch)
        new_tgt_vt      = jnp.where(do_switch_for_target, sampled_vt,     state.target_vt)
        ...
        state = state.replace(
            ...
            last_check_time=jnp.where(do_switch_for_target, jnp.broadcast_to(state.time, (B,)), state.last_check_time),
