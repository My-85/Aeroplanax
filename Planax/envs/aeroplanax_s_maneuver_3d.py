# envs/aeroplanax_s_maneuver_3d.py
# -*- coding: utf-8 -*-
"""
3D S-maneuver task environment.

Extends the 2D S-maneuver with altitude variation:
  waypoint_north = origin_n + idx * dn
  waypoint_east  = origin_e + amplitude * sin(pi * idx / points_per_half)
  waypoint_alt   = origin_alt + alt_amplitude * sin(pi * idx / alt_points_per_half)

target_pitch = arctan2(delta_alt, max(hdist, 1e-6))
Reach判定 uses 3D distance: sqrt(dn^2 + de^2 + da^2) <= reach_radius
"""
...
@struct.dataclass
class S_Maneuver_3D_TaskState(EnvState):
...
class S_Maneuver_3D_TaskParams(EnvParams):
    num_allies: int = 1
    num_enemies: int = 0
    num_missiles: int = 0
    agent_type: int = 0
    ...
    # 3D altitude parameters
    s_altitude_amplitude: float = 1000.0
    s_alt_points_per_half: int = 60


class AeroPlanaxSManeuver3DEnv(AeroPlanaxEnv[S_Maneuver_3D_TaskState, S_Maneuver_3D_TaskParams]):
    def __init__(self, env_params: Optional[S_Maneuver_3D_TaskParams] = None):
    ...
    @property
    def default_params(self) -> S_Maneuver_3D_TaskParams:
        return S_Maneuver_3D_TaskParams()

    @functools.partial(jax.jit, static_argnums=(0,))
    def _term_s_maneuver_done(self, state: S_Maneuver_3D_TaskState,
                               params: S_Maneuver_3D_TaskParams, agent_id: AgentID):
    ...
    def _init_state(
        self,
        key: chex.PRNGKey,
        params: S_Maneuver_3D_TaskParams,
    ) -> S_Maneuver_3D_TaskState:
        ...
        s_origin_alt = state.plane_state.altitude

        # Compute first waypoint (idx=1)
        ...
        wp_alt = s_origin_alt + params.s_altitude_amplitude * jnp.sin(
            jnp.pi * 1.0 / params.s_alt_points_per_half
        )

        delta_n = wp_n - state.plane_state.north
        delta_e = wp_e - state.plane_state.east
        delta_alt = wp_alt - state.plane_state.altitude
        init_target_heading = jnp.arctan2(delta_e, delta_n)
        hdist = jnp.sqrt(delta_n ** 2 + delta_e ** 2)
        init_target_pitch = jnp.arctan2(delta_alt, jnp.maximum(hdist, 1e-6))

        target_vt = jnp.full((self.num_agents,), params.s_target_vt)

        extra_state = jnp.stack([init_target_heading, init_target_pitch, target_vt], axis=0)

        state = S_Maneuver_3D_TaskState.create(
    ...
    def _reset_task(
        ...
        hdist = jnp.sqrt(delta_n ** 2 + delta_e ** 2)
        target_pitch = jnp.arctan2(delta_alt, jnp.maximum(hdist, 1e-6))
    ...
    def _step_task(
        ...
        params: S_Maneuver_3D_TaskParams,
    ) -> Tuple[S_Maneuver_3D_TaskState, Dict[str, Any]]:
        """Waypoint tracking along 3D S-curve. Advance waypoint when reached."""
        ...
        wp_alt = state.s_origin_alt + params.s_altitude_amplitude * jnp.sin(
            jnp.pi * idx_f / params.s_alt_points_per_half
        )
        ...
        delta_e = wp_e - state.plane_state.east
        delta_alt = wp_alt - state.plane_state.altitude
        dist3d = jnp.sqrt(delta_n ** 2 + delta_e ** 2 + delta_alt ** 2)

        # Reached?
        reached = dist3d <= params.reach_radius
        ...
        # Compute heading and pitch to NEW waypoint (after potential advance)
        ...
        new_wp_alt = state.s_origin_alt + params.s_altitude_amplitude * jnp.sin(
            jnp.pi * new_idx_f / params.s_alt_points_per_half
        )
        new_delta_n = new_wp_n - state.plane_state.north
        new_delta_e = new_wp_e - state.plane_state.east
        new_delta_alt = new_wp_alt - state.plane_state.altitude
        new_target_heading = jnp.arctan2(new_delta_e, new_delta_n)
        new_hdist = jnp.sqrt(new_delta_n ** 2 + new_delta_e ** 2)
        new_target_pitch = jnp.arctan2(new_delta_alt, jnp.maximum(new_hdist, 1e-6))

        # Update heading_turn_counts on reach
