...
def plot_comparison(planax_traj, jsbsim_traj, scenario, output_dir):
    ...
    plt.tight_layout()
    out = os.path.join(output_dir, f'{scenario}_comparison_v3.png')
...
# ============================================================
def main():
    output_dir = Path(__file__).parent.parent / "results" / "fidelity_validation_fixed"
    ...
    for sc in scenarios:
        ...
        save_csv(ptraj, output_dir / f'{sc}_planax_v3.csv')
        save_csv(jtraj, output_dir / f'{sc}_jsbsim_v3.csv')
    ...
    with open(output_dir / 'metrics_summary_v3.json', 'w') as f:
        json.dump(all_metrics, f, indent=2)
    print(f"\nMetrics saved to {output_dir / 'metrics_summary_v3.json'}")

    latex_txt = generate_latex_table(all_metrics, str(output_dir / 'validation_table_v3.tex'))

    # --- Step 4: Markdown summary ---
    summary = output_dir / 'validation_summary_v3.md'
