...
CKPT_PATH = os.path.abspath(
    # "results/heading_pitch_V_discrete_rnn_2026-05-09-16-53/checkpoints/checkpoint_epoch_300" # 没加速度制动板的baseline
    "results/heading_pitch_V_discrete_rnn_2026-05-14-15-29/checkpoints/checkpoint_epoch_300" # 加速度制动板的baseline
)  # Update this once Euler training finishes!
OUTPUT_DIR = "results/waypoint_s_euler"
SEED = 42
MAX_STEPS = 5000
...
MAX_WAYPOINTS       = 100
REACH_RADIUS        = 1000.0    # was 500 — wider capture for heading-only guidance
CRUISE_VT           = 250.0
...
def main():
    ...
    wp_reached_times = []
    wp_switch_step = 0
    wp_locked_heading = 0.0  # initialized at step 0
    ...
    for step in range(MAX_STEPS):
        ...
        # Direct bearing to waypoint, locked at switch time (prevents chasing)
        if step == wp_switch_step:
            wp_locked_heading = float(np.arctan2(d_e, d_n))
        target_heading = wp_locked_heading
