...
class Transition(NamedTuple):
    ...
    obs: jnp.ndarray
    info: Dict[str, jnp.ndarray] # 修正 Transition 的 info 类型，避免 JIT 里把 dict 当成数组导致潜在不一致。
...
def make_train(config):
    ...
    if "LOADDIR" in cfg:
        ...
        init_x = (
            jnp.zeros((1, cfg["NUM_ENVS"] * cfg["NUM_ACTORS"], obs_dim)),
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, _):
            ...
            ratio_mean = jnp.mean(loss_info["ratio"])

            metric = traj_batch.info  # 环境返回的 episodic/计数等
            metric["loss"] = loss_mean
            metric["loss"]["ratio_mean"] = ratio_mean
            ...
            if cfg.get("DEBUG"):
                def callback(m):
                    ...
                    # wandb: PPO损失/调度
                    wb_payload = {
                        "loss/total": float(m["loss"]["total_loss"]),
                        "loss/value": float(m["loss"]["value_loss"]),
                        "loss/actor": float(m["loss"]["actor_loss"]),
                        "loss/entropy": float(m["loss"]["entropy"]),
                        "sched/target_kl_eff": float(m["target_kl_eff"]),
                        "sched/ent_coef":      float(m["ent_coef"]),
                        "sched/lr_mult":       float(m["lr_mult"]),
                        "sched/kl_stop":       float(m["kl_stop"]),
                        "loss/ratio_mean":     float(m["loss"]["ratio_mean"]),
                    }

                    # wandb: 航点奖励分量与指标（若存在则记录）
                    for k, tag in [
                        ("reward_distance",        "reward/distance"),
                        ("reward_alignment",       "reward/alignment"),
                        ("reward_speed_profile",   "reward/speed_profile"),
                        ("reward_reach_bonus",     "reward/reach_bonus"),
                        ("penalty_crash",          "reward/penalty_crash"),
                        ("dist_to_wp",             "waypoint/dist"),
                        ("hdist_to_wp",            "waypoint/hdist"),
                        ("reach_radius",           "waypoint/reach_radius"),
                        ("reached_count",          "waypoint/reached_count"),
                        ("reached_this_step",      "waypoint/reached_flag_rate"),
                    ]:
                        if k in m:
                            v = m[k]
                            try:
                                v_mean = float(jnp.mean(v))
                                wb_payload[tag] = v_mean
                            except Exception:
                                pass

                    wandb.log(wb_payload, step=env_steps)

                    # TensorBoard（保持原有）
                    for k, v in m["loss"].items():
                        writer.add_scalar(f"loss/{k}", float(v), env_steps)

                    if "returned_episode" in m:
                        indexs = m["returned_episode"]
                        if indexs.sum() > 0:
                            writer.add_scalar('eval/episodic_return',
                                              float(m["returned_episode_returns"][indexs].mean()), env_steps)
                            writer.add_scalar('eval/episodic_length',
                                              float(m["returned_episode_lengths"][indexs].mean()), env_steps)
...
config = {
    "GROUP": "waypoint(rnn_critic_no_fc2_seed42)",
    "SEED": 42,
    "FOR_LOOP_EPOCHS": 1,
    "LR": 3e-4,
    "NUM_ENVS": 1000,
