"""
render_s_maneuver.py
====================
Load heading_pitch_V baseline checkpoint, fly S-maneuver via waypoint tracking,
output .acmi file with all waypoints batch-written for Tacview visualization.

Usage:
  cd Planax && python render_s_maneuver.py
"""
...
from envs.aeroplanax_s_maneuver import (
    AeroPlanaxSManeuverEnv,
    S_Maneuver_TaskParams,
)
...
def compute_s_waypoints(origin_n, origin_e, origin_alt, params):
    """Pre-compute all S-curve waypoints (numpy, for ACMI batch write)."""
...
def render_episode(config):
    # ---------- 1) Environment ----------
    env_params = S_Maneuver_TaskParams()
    env_core = AeroPlanaxSManeuverEnv(env_params)
    ...
    # ---------- 5) Pre-compute S-curve waypoints & write ACMI ----------
    origin_n = _f(core_state.s_origin_n)
    origin_e = _f(core_state.s_origin_e)
    origin_alt = _f(core_state.plane_state.altitude)
    s_waypoints = compute_s_waypoints(origin_n, origin_e, origin_alt, env_params)

    logdir = config.get("LOGDIR", "./tracks/s_maneuver/")
    ...
    print(f"\n{'='*100}")
    print(f"S-maneuver render: amplitude={env_params.s_amplitude}m, "
    ...
    print(f"\n{'Step':>6} | {'WP_idx':>6} | {'Reached':>7} | {'Hdist':>8} | "
          f"{'Heading':>8} | {'TgtHdg':>8} | {'dH':>8} | {'Vt':>6} | {'Alt':>7} | {'R':>7}")
    print(f"{'-'*100}")
    ...
    for step in range(max_steps):
        ...
        hdist_val = float(np.sqrt(
            (_f(core_state.plane_state.north) - (origin_n + wp_idx * env_params.s_half_period_north / env_params.s_points_per_half)) ** 2 +
            (_f(core_state.plane_state.east) - (origin_e + env_params.s_amplitude * np.sin(np.pi * wp_idx / env_params.s_points_per_half))) ** 2
        ))
        ...
        if step % 50 == 0 or wp_reached > 0 and step % 20 == 0:
            print(
                f"{step:6d} | {wp_idx:6d} | {wp_reached:7d} | {hdist_val:8.1f} | "
...
if __name__ == "__main__":
    config = {
        ...
        "ACTIVATION": "relu",
        "MAX_STEPS": 5000,
        "LOGDIR": "./tracks/s_maneuver/",
        "LOADDIR": os.path.abspath(
            "/home/dqy/aeroplanax/new/20251215最新代码库/results/heading_pitch_V_discrete_rnn_2026-03-16-10-26/checkpoints/checkpoint_epoch_600"
        ),
    }
