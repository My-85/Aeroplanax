        ...
        def _update_step(update_runner_state, _):
            ...
            if cfg.get("DEBUG"):
                def callback(m):
                    ...
                    # 新增：竖直目标累计完成次数
                    writer.add_scalar('eval/vertical_success',
                                      float(m["vertical_success_counts"][m["returned_episode"].squeeze()].mean()), env_steps)
...
config = {
    "GROUP": "lczh_baseline(new_rnn_6e8_warm_start_reward_clip_seed42)",
    "SEED": 42,
    "FOR_LOOP_EPOCHS": 1,
    "LR": 3e-4,
    "NUM_ENVS": 1000,
