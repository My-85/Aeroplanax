...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, _):
            ...
            metric["reward/dbg_wp_dist_km"]      = _mean("dbg_wp_dist_km")
            metric["reward/wp_reached"]           = _mean("waypoints_reached")
            # ---------------------------------------------------------------
            ...
            if cfg.get("DEBUG"):
                def callback(m):
                    ...
                    writer.add_scalar('eval/success_times',
                                      float(m["waypoints_reached"][m["returned_episode"].squeeze()].mean()), env_steps)
                    # 调度
                    ...
                    succ   = float(m["waypoints_reached"][m["returned_episode"].squeeze()].mean())
                    ...
                    wp_dist = float(m.get("reward/dbg_wp_dist_km", 0.0))
                    print("EnvStep={:<10} EpLen={:<6.2f} Return={:<8.2f} WP={:.0f} | "
                          "r_av={:.3f} r_alt={:.3f} r_cr={:+.0f} | "
                          "h_err={:.1f}° p_err={:.1f}° r_err={:.1f}° s_err={:.1f}m/s | "
                          "wp={:.1f}km".format(
                        env_steps, ep_len, ep_ret, succ,
                        r_av, r_alt, r_cr, h_err, p_err, r_deg, s_err, wp_dist,
                    ))
...
config = {
    ...
    "OUTPUTDIR": "results/" + "waypoint" + "_" + str_date_time,
    "LOGDIR": "results/" + "waypoint" + "_" + str_date_time + "/logs",
    "SAVEDIR": "results/" + "waypoint" + "_" + str_date_time + "/checkpoints",
    # Load Euler-angle baseline — same architecture (22-dim obs, 5 action heads)
    "LOADDIR": os.path.abspath("results/heading_pitch_V_discrete_rnn_2026-05-14-15-29/checkpoints/checkpoint_epoch_300"),
}
