...
print(f'combat_hierarchy policy: load heading_pitch_V model from {os.path.join(os.getcwd(),"/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/2v2_lczh_mine/AeroPlanax_multi_combat_2v2/envs/models/RNN新策略/PPO+RNN(actor与critic均加了layer_norm且改进训练过程（暖启动+防梯度消失+reward_clip)/seed42/heading_pitch_V_discrete_rnn_2025-08-29-15-55/checkpoints/checkpoint_epoch_600")}')
config = {
    "SEED": 42,
    "LR": 3e-4,
    "NUM_ENVS": 1,
    ...
    "LOADDIR": os.path.join(os.getcwd(),"/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/2v2_lczh_mine/AeroPlanax_multi_combat_2v2/envs/models/RNN新策略/PPO+RNN(actor与critic均加了layer_norm且改进训练过程（暖启动+防梯度消失+reward_clip)/seed42/heading_pitch_V_discrete_rnn_2025-08-29-15-55/checkpoints/checkpoint_epoch_600")
}
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        ################################################################
        # 我的代码，这里加了一层，补回瓶颈层提升表达力

        nn_fc2 = nn.Dense(256, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(embedding)
        nn_fc2 = nn.LayerNorm()(nn_fc2) # 加一层归一化，稳定分布，减小梯度震荡，特别适合小 batch、RNN/Transformer。它不引入非线性。
        nn_fc2 = activation(nn_fc2)
        ...
        # “构造分布”(distrax.Categorical(logits=...)
...
# 兼容多版本 Orbax 的恢复方式
try:
    restored = ckptr.restore(config['LOADDIR'], item=state)
except Exception:
    try:
        restored = ckptr.restore(config['LOADDIR'], args=ocp.args.StandardRestore(item=state))
    except Exception:
        restored = ckptr.restore(config['LOADDIR'])  # 老版本可能直接返回保存时的 dict

# 兼容不同保存键名
if isinstance(restored, dict) and "params" in restored:
    controller_params = restored["params"]
elif isinstance(restored, dict) and "actor_params" in restored:
    controller_params = restored["actor_params"]
else:
    controller_params = restored  # 已经是参数 pytree
...
class HierarchicalCombatTaskParams(EnvParams):
    ...
    # 仅作用于敌方，分散其初始位置与朝向
    enemy_team_spacing_scale: float = 3.0
    enemy_yaw_spread_deg: float = 60.0

    safe_distance: float = 100
    posture_reward_scale: float = 100.0
    use_baseline: bool = True

    comm_radius: float = 50000 # 通信半径(同阵营数据链共享)
    find_radius: float = 50000 # 发现半径(自机传感器直观测，等价“雷达发现”)
...
class AeroPlanaxHierarchicalCombatEnv(AeroPlanaxEnv[HierarchicalCombatTaskState, HierarchicalCombatTaskParams]):
    ...
    def _reset_task(
        ...
        # 友机朝向保持 0；敌机以 π 为基准做扇形散布
        ally_yaw = jnp.zeros((self.num_allies,))
        spread = jnp.deg2rad(params.enemy_yaw_spread_deg)
        # 在 [-spread, +spread] 之间均匀散布敌机朝向
        idx = jnp.linspace(-1.0, 1.0, self.num_enemies)
        enemy_yaw = jnp.pi + idx * spread

        yaw = jnp.concatenate([ally_yaw, enemy_yaw])
    ...
    def _step_task(
        ...
        r_evt_cus = jax.vmap(self.reward_functions[6], in_axes=(None, None, 0))(state, params, agent_ids)
        r_evt_raw = jax.vmap(self.reward_functions[7], in_axes=(None, None, 0))(state, params, agent_ids)
    ...
    def _get_obs(
        ...
        visible_mask = compute_visibility_mask(pos, self.num_allies, comm_radius=params.comm_radius, find_radius=params.find_radius)
    ...
    def _generate_formation(
        ...
        if self.formation_type == 0:
            ...
            enemy_positions = wedge_formation(self.num_enemies, params.team_spacing * params.enemy_team_spacing_scale)
        elif self.formation_type == 1:
            ally_positions = line_formation(self.num_allies, params.team_spacing)
            enemy_positions = line_formation(self.num_enemies, params.team_spacing * params.enemy_team_spacing_scale)
        elif self.formation_type == 2:
            ally_positions = diamond_formation(self.num_allies, params.team_spacing)
            enemy_positions = diamond_formation(self.num_enemies, params.team_spacing * params.enemy_team_spacing_scale)
        else:
