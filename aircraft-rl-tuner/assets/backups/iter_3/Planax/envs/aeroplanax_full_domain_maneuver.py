...
class FullDomain_TaskParams(EnvParams):
    ...
    nz_hard_limit: float = 20.0          # v13: raised to 20G — 12G caused 119 crashes/ep, normal maneuvers exceed 12G single-axis
...
class AeroPlanaxFullDomainEnv(AeroPlanaxEnv[FullDomain_TaskState, FullDomain_TaskParams]):
    ...
    def step(
        ...
        def update_status(plane_states):
            ...
            # v9: max_overload raised 12→16G to reduce Nz crashes during aggressive
            # attitude corrections. check_overload tests per-axis |ax|, |ay|, |az| >= limit.
            crashed = jax.vmap(
                functools.partial(check_crashed_full_domain, max_overload=16.0),
                in_axes=(None, 0)
