...
Target quaternion = conj(_quat_from_euler_nb(...)) to match dynamics state convention.
...
# ---- quaternion helpers ----
# Convention (consistent with reference env and full-domain env):
#   _quat_from_euler_nb: raw ZYX Euler-to-quaternion (same formula as env helpers)
#   target = conj(_quat_from_euler_nb(...)) to match dynamics state convention.
...
def unreach_full_domain_fn(
    ...
    Changes from original:
      - min_elapsed_sec lowered from 8 to 5 (faster feedback)
      - max_interval_sec raised from 20 to 30 (more time for hard targets)
    ...
    base_steps = getattr(params, 'sustained_on_target_steps', 3)
    per_level = getattr(params, 'sustained_on_target_per_level', 3)
