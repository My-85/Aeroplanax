...
REWARD_CONFIG = {
    "gaussian_scale_coarse_deg": 20.0,
    "gaussian_scale_medium_deg": 10.0,
    "gaussian_scale_fine_deg": 4.0,
    "gaussian_weight_coarse": 0.3,
    "gaussian_weight_medium": 0.4,
    "gaussian_weight_fine": 0.3,
    "speed_sigma": 30.0,
    # v11: restore att-dominant weights. v10's 0.60/0.40 let agent exploit speed-only
    # tracking (theta=87°, delta_vt=6 → r_tracking=0.38/step + spd_bonus=1.0 = 1.39/step).
    # With 0.75/0.25, speed-only gives 0.25*0.96=0.24/step — not enough to be an exploit.
    "track_weight_att": 0.75,
    "track_weight_spd": 0.25,
    "progress_coeff": 1.0,
    "progress_clip_lo": -0.5,
    "progress_clip_hi": 0.5,
    # v10: split on-target bonus into attitude-only, speed-only, and combined
    # Previously: full_bonus=4.0 required theta<10° AND delta_vt<25 simultaneously.
    # Agent couldn't achieve either → 4.0 bonus was completely inaccessible → no gradient.
    # Now: attitude bonus (1.5) for theta<10° alone, speed bonus (1.0) for delta_vt<15 alone,
    # combined bonus (2.0) when both are met. Agent has stepwise incentive: att → spd → both.
    "on_target_att_bonus": 1.5,
    "on_target_att_thresh_deg": 10.0,
    "on_target_spd_bonus": 1.0,
    "on_target_spd_thresh": 15.0,
    "on_target_spd_att_gate_deg": 30.0,  # v11: speed bonus only when theta<30° (prevents speed-only exploit)
    "on_target_full_bonus": 2.0,
    "on_target_full_thresh_deg": 10.0,
    "on_target_full_vt_thresh": 25.0,
    # v10: close_bonus reduced 0.2→0.1 (further eliminate plateau at theta~15-18°)
    "on_target_close_bonus": 0.1,
    "on_target_close_thresh_deg": 20.0,
    "error_penalty_coeff": -0.2,
    "error_penalty_thresh_deg": 45.0,
    "error_extreme_coeff": -0.2,
    "error_extreme_thresh_deg": 75.0,
    "smoothness_omega_thresh": 3.0,
    "smoothness_coeff": -0.02,
    "smoothness_gate_deg": 180.0,
    "overload_omega_thresh": 8.0,
    "overload_coeff": -0.1,
    "alive_bonus": 0.0,
    "crash_penalty": -15.0,
    "reward_clip_lo": -15.0,
    "reward_clip_hi": 5.0,
    # v10: curriculum level reward scale — higher levels get more reward per step
    # to compensate for harder targets and prevent "avoid success" exploit
...
def full_domain_reward_fn(
    ...
    r_att = (_cfg["gaussian_weight_coarse"] * r_coarse
    ...
    r_tracking = _cfg["track_weight_att"] * r_att + _cfg["track_weight_spd"] * r_spd
